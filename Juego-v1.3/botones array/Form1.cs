...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        public void JugadasDificil(object sender, EventArgs e)
        {
            ...
            Random R = new Random();
            if ((index + 3) < 7 && (index+3) >= 0 ||
                (index + 3) < 14 && (index + 3) >= 7 ||
                (index + 3) < 21 && (index + 3) >= 14 ||
                (index + 3) < 28 && (index + 3) >= 21 ||
                (index + 3) < 35 && (index + 3) >= 28 ||
                (index + 3) < 42 && (index + 3) >= 35)
            {
                if (boton[index].BackColor == boton[index+1].BackColor && boton[index].BackColor == boton[index+2].BackColor && boton[index].BackColor == boton[index + 3].BackColor)
                {
                    if ((index+4) == 7 || (index + 4) == 14 || (index + 4) == 21 || (index + 4) == 28 || (index + 4) == 35 || (index + 4) == 42)
                    {
                        boton[index - 1].BackColor = Color.Red;
                    }
                    else
                    {
                        boton[index + 4].BackColor = Color.Red;
                    }
                }
            }
