...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        //La variable que va a permitir el cambio entre jugador y jugador
        int turno = 0;
        //La variable que va a permitir saber cuando gana el azul o rojo
        int ganadorcolor = 0;
        //Permite leer cuantas victorias hubo
        int GanoAzul = 0;
        int GanoRojo = 0;
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            int Loc1 = 430;
            turno = turno + 1;
            //Muestra y personaliza el texto dependiendo quien juega en la pantalla
            label1.Text = "Turno del jugador";
            label1.Enabled = true;
            label2.Text = "Azul";
            label2.Enabled = true;
        ...
        //Boton de robot muestra los botones de dificultad
        ...
        //jugadas de modo facil
        public async void JugadasFacil(object sender, EventArgs e)
        { 
            Button B = (Button)sender;
            int index = Array.IndexOf(boton, B);
        ...
        //jugadas de modo dificil
        public async void JugadasDificil(object sender, EventArgs e)
        {
            Button B = (Button)sender;
            int index = Array.IndexOf(boton, B);
            ...
            if (index < 28)
            {
                if (boton[index].BackColor == boton[index + 7].BackColor && boton[index].BackColor == boton[index + 14].BackColor && index > 7)
                {
                    boton[index - 7].BackColor = Color.Red;
                    boton[index - 7].Enabled = false;
                    if (index > 7 && boton[index - 14].BackColor != Color.Red)
                    {
        ...
        public async void Ganaste(object sender, EventArgs e)
        {
            for (int n = 0; n < 42; n++)
            {
                await Task.Delay(000001);
                boton[n].Enabled = false;
                if (ganadorcolor == 1)
                {
                    boton[n].BackColor = Color.Red;
                    label2.Text = "Gano el rojo";
                    label2.ForeColor = Color.DarkRed;
                    if (n == 0)
                    {
                        GanoRojo = GanoRojo + 1;
                    }
                    label9.Text = "" + GanoRojo;
                }
                else
                {
                    boton[n].BackColor= Color.DarkBlue;
                    label2.Text = "Gano el azul";
                    label2.ForeColor = Color.Blue;
                    if (n == 0)
                    {
                        GanoAzul = GanoAzul + 1;
                    }
                    label5.Text = "" + GanoAzul;
                }
                label1.Text = "Termino el juego";
            }
        }
        ...
        private async void button9_Click(object sender, EventArgs e)
        {
            ...
            button10.Visible = true;
             label6.Text = "En un videojuego muy lejano." +
                "\n4 en linea es un juego donde uno debe ganar haciendo una fila del mismo color de 4" +
                "\npero solo puedo colocar las fichas si el cuadrado de abajo esta ocupado." +
                "\nPara empezar debe elegir si quiere jugar con una persona o con un Bot(Robot)" +
                "\nEn el caso de una persona debe elegir la opcion Jugador vs Jugador" +
                "\nEn el otro caso la opcion vs robot, que contara con dos dificultades a elegir" +
                "\nel facil el bot jugara automaticamente y en dificil buscara bloquear sus jugadas" +
                "\nLas opciones de la derecha cuentan como ayuda en el caso de vovler al menu," +
                "\nReiniciar el puntaje," +
                "\nSalir del juego," +
                "\nSi se traba el juego y ocurre un error que puede pasar";
            //unicamente para referencia
            for (int n = 0; n < 2000000; n++)
            {
                await Task.Delay(100);
                label6.Top += 1;

                // Reducir tamaño de la fuente 1 punto
                if (label6.Font.Size >= 2)
                {
                    label6.Font = new Font(label6.Font.FontFamily, label6.Font.Size - 0.01f);
                }
            }
        }
